...
class behat_mod_individualfeedback extends behat_base {
    ...
    /**
     * Adds a question to the existing individualfeedback with filling the form.
     *
     * The form for creating a question should be on one page.
     *
     * @When /^I add a "(?P<question_type_string>(?:[^"]|\\")*)" question to the individualfeedback with:$/
     ...
     * The form for creating a question should be on one page.
     *
     * @When /^I add a page break to the individualfeedback$/
     */
    public function i_add_a_page_break_to_the_individualfeedback() {

        $questiontype = $this->escape(get_string('add_pagebreak', 'individualfeedback'));
        $this->execute('behat_general::i_click_on', [get_string('add_item', 'mod_individualfeedback'), 'link']);
        $this->execute('behat_general::i_click_on', [$questiontype, 'link']);
    ...
    /**
     * Quick way to generate answers to a one-page individualfeedback.
     *
     * @When /^I log in as "(?P<user_name_string>(?:[^"]|\\")*)" and complete individualfeedback "(?P<individualfeedback_name_string>(?:[^"]|\\")*)" in course "(?P<course_name_string>(?:[^"]|\\")*)" with:$/
     * @param string $questiontype
     * @param TableNode $questiondata with data for filling the add question form
     */
    public function i_log_in_as_and_complete_individualfeedback_in_course($username, $individualfeedbackname, $coursename, TableNode $answers) {
        $username = $this->escape($username);
        $coursename = $this->escape($coursename);
        $individualfeedbackname = $this->escape($individualfeedbackname);
        $completeform = $this->escape(get_string('complete_the_form', 'individualfeedback'));

        // Log in as user.
        $this->execute('behat_auth::i_log_in_as', $username);

        // Navigate to individualfeedback complete form.
        $this->execute('behat_navigation::i_am_on_page_instance', [$individualfeedbackname, 'individualfeedback activity']);
    ...
    /**
     * Exports individualfeedback and makes sure the export file is the same as in the fixture
     *
     * @Then /^following "(?P<link_string>(?:[^"]|\\")*)" should export individualfeedback identical to "(?P<filename_string>(?:[^"]|\\")*)"$/
     * @param string $link
     ...
     *
     * @Then /^I show chart data for the "(?P<individualfeedback_name_string>(?:[^"]|\\")*)" individualfeedback$/
     * @param string $individualfeedbackname name of the individualfeedback for which chart data needs to be shown.
     */
    public function i_show_chart_data_for_the_individualfeedback($individualfeedbackname) {

        $individualfeedbackxpath = "//th[contains(normalize-space(string(.)), \"" . $individualfeedbackname . "\")]/ancestor::table//" .
        ...
        $charttabledataxpath = $individualfeedbackxpath .
    ...
    /**
     * Ensures two individualfeedback export files are identical
     *
