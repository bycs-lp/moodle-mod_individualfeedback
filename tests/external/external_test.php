...
final class external_test extends \core_external\tests\externallib_testcase {
     ...
     * @param array $info Settings to create the individualfeedback.
    ...
    public function test_get_items_validation(string $role, array $info, ?string $warning): void {
        ...
        if (!array_key_exists('empty', $info) || !$info['empty']) {
            ...
            $individualfeedbackgenerator = $this->getDataGenerator()->get_plugin_generator('mod_individualfeedback');
            // Add,at least, one item to the individualfeedback.
    ...
    public static function items_provider(): array {
        return [
            'Valid individualfeedback (as student)' => [
                'role' => 'student',
                'info' => [],
                'warning' => null,
            ],
            'Closed individualfeedback (as student)' => [
                'role' => 'student',
                'info' => ['closed' => true],
                'warning' => get_string('individualfeedback_is_not_open', 'individualfeedback'),
            ],
            'Empty individualfeedback (as student)' => [
                'role' => 'student',
                'info' => ['empty' => true],
                'warning' => get_string('no_items_available_yet', 'individualfeedback'),
            ],
            'Cannot complete individualfeedback (as student)' => [
                'role' => 'student',
                'info' => ['complete' => false],
                'warning' => 'required_capability_exception',
            ],
            'Valid individualfeedback (as teacher)' => [
                'role' => 'teacher',
                'info' => [],
                'warning' => null,
            ],
            'Closed individualfeedback (as teacher)' => [
                'role' => 'teacher',
                'info' => ['closed' => true],
                'warning' => null,
            ],
            'Empty individualfeedback (as teacher)' => [
                'role' => 'teacher',
                'info' => ['empty' => true],
                'warning' => null,
            ],
            'Cannot complete individualfeedback (as teacher)' => [
                'role' => 'teacher',
                'info' => ['complete' => false],
                'warning' => null,
            ],
    ...
    /**
     * Test process_page for a site individualfeedback.
     */
    public function test_process_page_site_individualfeedback(): void {
        global $DB;
        $pagecontents = 'You finished it!';
        $this->individualfeedback = $this->getDataGenerator()->create_module('individualfeedback',
     ...
     */
    public function test_get_non_respondents_from_anonymous_individualfeedback(): void {
        $this->setUser($this->student);
        $this->expectException(moodle_exception::class);
    ...
    /**
     * Test get_individualfeedback_access_information for site individualfeedback.
     */
    public function test_get_individualfeedback_access_information_for_site_individualfeedback(): void {

        $siteindividualfeedback = $this->getDataGenerator()->create_module('individualfeedback', array('course' => SITEID));
        $this->setUser($this->student);
        // Access the site individualfeedback via the site activity.
        $result = mod_individualfeedback_external::get_individualfeedback_access_information($siteindividualfeedback->id);
        $result = external_api::clean_returnvalue(mod_individualfeedback_external::get_individualfeedback_access_information_returns(), $result);
        $this->assertTrue($result['cancomplete']);
        $this->assertTrue($result['cansubmit']);

        // Access the site individualfeedback via course where I'm enrolled.
        $result = mod_individualfeedback_external::get_individualfeedback_access_information($siteindividualfeedback->id, $this->course->id);
        $result = external_api::clean_returnvalue(mod_individualfeedback_external::get_individualfeedback_access_information_returns(), $result);
        $this->assertTrue($result['cancomplete']);
        $this->assertTrue($result['cansubmit']);

        // Access the site individualfeedback via course where I'm not enrolled.
        $othercourse = $this->getDataGenerator()->create_course();

        $this->expectException(moodle_exception::class);
        mod_individualfeedback_external::get_individualfeedback_access_information($siteindividualfeedback->id, $othercourse->id);
    }

    /**
     * Test get_individualfeedback_access_information for site individualfeedback mapped.
     */
    public function test_get_individualfeedback_access_information_for_site_individualfeedback_mapped(): void {
        ...
        $siteindividualfeedback = $this->getDataGenerator()->create_module('individualfeedback', array('course' => SITEID));
        $this->setUser($this->student);
        $DB->insert_record('individualfeedback_sitecourse_map', array('individualfeedbackid' => $siteindividualfeedback->id, 'courseid' => $this->course->id));

        // Access the site individualfeedback via course where I'm enrolled and mapped.
        $result = mod_individualfeedback_external::get_individualfeedback_access_information($siteindividualfeedback->id, $this->course->id);
        $result = external_api::clean_returnvalue(mod_individualfeedback_external::get_individualfeedback_access_information_returns(), $result);
        $this->assertTrue($result['cancomplete']);
        $this->assertTrue($result['cansubmit']);

        // Access the site individualfeedback via course where I'm enrolled but not mapped.
        $othercourse = $this->getDataGenerator()->create_course();
