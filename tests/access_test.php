...
final class access_test extends \advanced_testcase {
    ...
    /**
     * Submit individualfeedback response.
     * @param \stdClass $individualfeedback Mod_individualfeedback instance.
     ...
     */
    private function student_response(\stdClass $individualfeedback, \stdClass $cm, int $itemid, int $userid): mod_individualfeedback_completion {
        $completion = new mod_individualfeedback_completion($individualfeedback, $cm,
    ...
    /**
     * Test access to individualfeedback responses is allowed or denied correctly for
     ...
     * should be able to access the submitted individualfeedback, for example,
    ...
    public function test_response_access(int $groupmode, int $anonymous, array $studentgroups, array $teacheraccesses): void {
        ...
        $individualfeedback = $this->getDataGenerator()->create_module('individualfeedback', ['course' => $course->id, 'anonymous' => $anonymous]);
        ...
        $completion = $this->student_response($individualfeedback, $cm, $item->id, $student->id);
        ...
        foreach ($teacheraccesses as $teacher => $access) {
            ...
            // access.
            try {
                new mod_individualfeedback_completion($individualfeedback, $cm, 0, true, $showcompleted, $userid);
