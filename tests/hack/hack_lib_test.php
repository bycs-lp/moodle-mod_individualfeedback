...
final class hack_lib_test extends core_hack_testcase {
    ...
    public function test_get_group_values_selfassessment_filter(): void {
        ...
        // Signature: ($item, $groupid, $courseid, $ignore_empty, $selfassessment).
        $normal = individualfeedback_get_group_values($item, false, $course->id, false, false);
        $this->assertCount(1, $normal);
        $this->assertSame('normal', reset($normal)->value);

        // Requesting selfassessment=true must return only the self-assessment record of
        // the current user (admin, id 2 — matches the inserted userid).
        $this->setAdminUser();
        $self = individualfeedback_get_group_values($item, false, $course->id, false, true);
        $this->assertCount(1, $self);
        $this->assertSame('selfassess', reset($self)->value);

        // Another user has no self-assessment record.
        $other = $this->getDataGenerator()->create_user();
        $this->setUser($other);
        $this->assertCount(0, individualfeedback_get_group_values($item, false, $course->id, false, true));
    }

    /**
     * The group filter of the original function must survive the hack: with a groupid only
     * values of group members are returned (regression found by core behat groups.feature).
     *
     * @covers \mod_individualfeedback\hack\lib::get_values_by_selfassessment
     */
    public function test_get_group_values_respects_groupid(): void {
        global $DB;
        $this->resetAfterTest();
        $this->setAdminUser();
        ...
        $individualfeedback = $generator->create_module(
            'individualfeedback',
            ['course' => $course->id, 'anonymous' => INDIVIDUALFEEDBACK_ANONYMOUS_NO]
        );
        $plugingenerator = $generator->get_plugin_generator('mod_individualfeedback');
        $item = $plugingenerator->create_item_textfield($individualfeedback);

        $group1 = $generator->create_group(['courseid' => $course->id]);
        $group2 = $generator->create_group(['courseid' => $course->id]);
        $users = [];
        foreach ([$group1, $group1, $group2] as $i => $group) {
        ...
        // No group: all three values.
        $this->assertCount(3, individualfeedback_get_group_values($item, false, $course->id));
        // Group 1: two values, group 2: one value.
        $this->assertCount(2, individualfeedback_get_group_values($item, $group1->id, $course->id));
        $this->assertCount(1, individualfeedback_get_group_values($item, $group2->id, $course->id));

        // Same filtering in core-test mode (original code path) — proves equivalence.
        \mod_individualfeedback\hack\core_hack::stop_hack_test();
        $this->assertCount(2, individualfeedback_get_group_values($item, $group1->id, $course->id));
        \mod_individualfeedback\hack\core_hack::start_hack_test();
    }
}
