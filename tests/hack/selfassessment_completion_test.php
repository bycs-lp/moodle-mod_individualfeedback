...
final class selfassessment_completion_test extends core_hack_testcase {
    ...
    // -------------------------------------------------------------------------
    // H21 — completion::create_current_completed_tmp() / flag_selfassessment_completedtmp().
    // -------------------------------------------------------------------------

    /**
     * Submit one answer for the given user through the completion API and return the
     * resulting individualfeedback_completed record.
     *
     * @param \stdClass $cm Course module record.
     * @param int $userid The responding user.
     * @return \stdClass The completed record.
     */
    private function submit_response(\stdClass $cm, int $userid): \stdClass {
        global $DB;

        $individualfeedback = $DB->get_record('individualfeedback', ['id' => $cm->instance], '*', MUST_EXIST);
        $item = $DB->get_record('individualfeedback_item', ['individualfeedback' => $individualfeedback->id]);
        if (!$item) {
        ...
        $completion->save_response_tmp((object) ['textfield_' . $item->id => 'answer']);
        $completion->save_response();
        return $completion->get_completed();
    }

    /**
     * Production mode: a response of a user holding mod/individualfeedback:selfassessment
     * (editing teacher by default) is stored with selfassessment = 1, a student's with 0.
     *
     * Fails if the injection line in completion::create_current_completed_tmp() is lost,
     * because the column then keeps its default 0 for the teacher.
     */
    public function test_selfassessment_flag_set_on_submission_in_production_mode(): void {
        $this->resetAfterTest();
        [, $cm, $context, $student, $teacher] = $this->setup_feedback_with_teacher();

        $this->assertTrue(has_capability('mod/individualfeedback:selfassessment', $context, $teacher->id));
        $this->assertFalse(has_capability('mod/individualfeedback:selfassessment', $context, $student->id));

        $teachercompleted = $this->submit_response($cm, $teacher->id);
        $this->assertSame(
            1,
            (int) $teachercompleted->selfassessment,
            'Teacher response must be flagged as self-assessment in production mode.'
        );

        $studentcompleted = $this->submit_response($cm, $student->id);
        $this->assertSame(
            0,
            (int) $studentcompleted->selfassessment,
            'Student response must not be flagged as self-assessment.'
        );

        // The flag survives an edit of the response (tmp record recreated from the last completed).
        $this->submit_response($cm, $teacher->id);
        $this->assertSame(1, (int) $this->submit_response($cm, $teacher->id)->selfassessment);
    }

    /**
     * Core-test mode: the hack returns the record unchanged, so even the teacher's response
     * keeps selfassessment = 0 (original mod_feedback behaviour).
     */
    public function test_selfassessment_flag_not_set_in_core_test_mode(): void {
        $this->resetAfterTest();
        [, $cm, , , $teacher] = $this->setup_feedback_with_teacher();

        core_hack::stop_hack_test();
        try {
            $completed = $this->submit_response($cm, $teacher->id);
            $this->assertSame(
                0,
                (int) $completed->selfassessment,
                'In core-test mode the selfassessment flag must stay 0.'
            );
        } finally {
            core_hack::start_hack_test();
        }
