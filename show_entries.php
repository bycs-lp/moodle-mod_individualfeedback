...
if ($deleteid) {
    ...
    require_sesskey();
    $feedbackstructure = new mod_individualfeedback_completion($feedback, $cm, 0, true, $deleteid);
    individualfeedback_delete_completed($feedbackstructure->get_completed(), $feedback, $cm);
    redirect($baseurl);
} else if ($showcompleted || $userid) {
    // Viewing individual response.
    $feedbackstructure = new mod_individualfeedback_completion($feedback, $cm, 0, true, $showcompleted, $userid);
} else {
    // Viewing list of reponses.
    $feedbackstructure = new mod_individualfeedback_structure($feedback, $cm, $courseid);
}

$responsestable = new mod_individualfeedback_responses_table($feedbackstructure);
$anonresponsestable = new mod_individualfeedback_responses_anon_table($feedbackstructure);
...
$courseselectform = new mod_individualfeedback_course_select_form($baseurl, $feedbackstructure, $feedback->course == SITEID);
...
if ($userid || $showcompleted) {
    ...
    $form = new mod_individualfeedback_complete_form(mod_individualfeedback_complete_form::MODE_VIEW_RESPONSE,
            $feedbackstructure, 'individualfeedback_viewresponse_form');
    $form->display();
