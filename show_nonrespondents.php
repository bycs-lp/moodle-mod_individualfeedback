...
if ($action == 'sendmessage' && $canbulkmessaging) {
    ...
    '<a target="_blank" href="'.$link2.'">'.$strindividualfeedbacks.'</a> &raquo; '.
    '<a target="_blank" href="'.$link3.'">'.format_string($individualfeedback->name, true).'</a>'.
    '</div>';
    ...
    if (is_array($messageuser)) {
        ...
        foreach ($messageuser as $userid) {
            ...
            $eventdata->contexturlname   = $individualfeedback->name;
...
$baseurl = new moodle_url('/mod/individualfeedback/show_nonrespondents.php');
...
if ($canbulkmessaging) {
    ...
    $togglercheckbox = new \core\output\checkbox_toggleall('individualfeedback-non-respondents', true, [
        'id' => $selectallid,
        'name' => $selectallid,
        'value' => 1,
...
// Return students record including if they started or not the individualfeedback.
...
if (empty($students)) {
    ...
    foreach ($students as $student) {
        ...
        if ($student->individualfeedbackstarted) {
    ...
    if ($canbulkmessaging) {
        ...
        echo '<label for="individualfeedback_subject">'.get_string('subject', 'individualfeedback').'&nbsp;</label>';
        echo '<input type="text" id="individualfeedback_subject" size="50" maxlength="255" name="subject" value="' .
        ...
        echo '<input type="submit" name="send_message" value="'.get_string('send', 'individualfeedback').'" class="btn btn-primary" />';
        echo '</div>';
