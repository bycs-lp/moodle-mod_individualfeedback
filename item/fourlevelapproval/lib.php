...
define('INDIVIDUALFEEDBACK_FOURLEVELAPPROVAL_TYPE_SEP', '>>>>>');
define('INDIVIDUALFEEDBACK_FOURLEVELAPPROVAL_LINE_SEP', '|');
define('INDIVIDUALFEEDBACK_FOURLEVELAPPROVAL_ADJUST_SEP', '<<<<<');
define('INDIVIDUALFEEDBACK_FOURLEVELAPPROVAL_IGNOREEMPTY', 'i');
define('INDIVIDUALFEEDBACK_FOURLEVELAPPROVAL_NEGATIVEFORMULATED', 'n');
define('INDIVIDUALFEEDBACK_FOURLEVELAPPROVAL_HIDENOSELECT', 'h');

class individualfeedback_item_fourlevelapproval extends individualfeedback_item_base {
    protected $type = "fourlevelapproval";

    public function build_editform($item, $feedback, $cm) {
        global $DB, $CFG;
        require_once('fourlevelapproval_form.php');

        // get the lastposition number of the individualfeedback_items
        $position = $item->position;
        $lastposition = $DB->count_records('individualfeedback_item', ['individualfeedback' => $feedback->id]);
        if ($position == -1) {
        ...
        if (empty($item->presentation)) {
            $item->presentation = 'r' . INDIVIDUALFEEDBACK_FOURLEVELAPPROVAL_TYPE_SEP . get_string('fourlevelapproval_options', 'individualfeedback');
        }
        ...
        $itemvalues = str_replace(INDIVIDUALFEEDBACK_FOURLEVELAPPROVAL_LINE_SEP, "\n", $item->presentation);
        ...
        // build the form
        $customdata = ['item' => $item,
                            'common' => $commonparams,
                            'positionlist' => $positionlist,
                            'position' => $position,
                            'info' => $info];

        $this->item_form = new individualfeedback_fourlevelapproval_form('edit_item.php', $customdata);
    }
    ...
    public function save_item() {
        ...
        return $DB->get_record('individualfeedback_item', ['id' => $item->id]);
    }
        ...
        if ($info->subtype == 'c') {
            ...
            for ($i = 1; $i <= $sizeofanswers; $i++) {
                ...
                foreach ($values as $value) {
                    ...
                    $vallist = explode(INDIVIDUALFEEDBACK_FOURLEVELAPPROVAL_LINE_SEP, $value->value);
    ...
    public function get_printval($item, $value) {
        ...
        $presentation = explode(INDIVIDUALFEEDBACK_FOURLEVELAPPROVAL_LINE_SEP, $info->presentation);

        if ($info->subtype == 'c') {
            $vallist = array_values(explode(INDIVIDUALFEEDBACK_FOURLEVELAPPROVAL_LINE_SEP, $value->value));
    ...
    public function excelprint_item(
        &$worksheet,
        $row_offset,
        $xls_formats,
        $item,
    ...
    /**
     * Options for the fourlevelapproval element
     * @param stdClass $item
     * @return array
     */
    protected function get_options($item) {
        ...
        foreach ($presentation as $idx => $optiontext) {
            $options[$idx + 1] = format_text($optiontext, FORMAT_HTML, ['noclean' => true, 'para' => false]);
        }
        if ($info->subtype === 'r' && !$this->hidenoselect($item)) {
            $options = [0 => get_string('not_selected', 'individualfeedback')] + $options;
        }
    ...
    public function complete_form_element($item, $form) {
        ...
        $class = 'fourlevelapproval-' . $info->subtype;
        ...
        if ($info->subtype === 'd' || ($info->subtype === 'r' && $form->is_frozen())) {
            ...
            $element = $form->add_form_element(
                $item,
                ['select', $inputname . '[0]', $name, [0 => ''] + $options, ['class' => $class]],
                false,
                false
            );
            ...
            $objs = [];
            foreach (explode(INDIVIDUALFEEDBACK_FOURLEVELAPPROVAL_LINE_SEP, $form->get_item_value($item)) as $v) {
            ...
            $class .= ' fourlevelapproval-' . ($info->horizontal ? 'horizontal' : 'vertical');
            $objs = [];
            if ($info->subtype === 'c') {
                // Checkboxes.
                ...
                foreach ($options as $idx => $label) {
                    $objs[] = ['advcheckbox', $inputname . '[' . $idx . ']', '', $label, null, [0, $idx]];
                ...
                $element = $form->add_form_group_element($item, 'group_' . $inputname, $name, $objs, $separator, $class);
                if ($tmpvalue) {
                    foreach (explode(INDIVIDUALFEEDBACK_FOURLEVELAPPROVAL_LINE_SEP, $tmpvalue) as $v) {
        ...
        if ($item->required) {
            ...
            $form->add_validation_rule(function ($values, $files) use ($elementname, $item) {
                ...
                return empty($values[$inputname]) || !array_filter($values[$inputname]) ?
                    [$elementname => get_string('required')] : true;
            });
        }
    }
    ...
    public function create_value($value) {
        ...
        return join(INDIVIDUALFEEDBACK_FOURLEVELAPPROVAL_LINE_SEP, $value);
    }
    ...
    public function compare_value($item, $dbvalue, $dependvalue) {
        ...
        if (is_array($dbvalue)) {
            ...
            $dbvalues = explode(INDIVIDUALFEEDBACK_FOURLEVELAPPROVAL_LINE_SEP, $dbvalue);
        }
    ...
    public function get_info($item) {
        ...
        $info = new stdClass();
        // check the subtype of the fourlevelapproval
        // it can be check(c), radio(r) or dropdown(d)
        $info->subtype = '';
        $info->presentation = '';
        $info->horizontal = false;

        $parts = explode(INDIVIDUALFEEDBACK_FOURLEVELAPPROVAL_TYPE_SEP, $item->presentation);
        @[$info->subtype, $info->presentation] = $parts;
        if (!isset($info->subtype)) {
            $info->subtype = 'r';
        }

        if ($info->subtype != 'd') {
            $parts = explode(INDIVIDUALFEEDBACK_FOURLEVELAPPROVAL_ADJUST_SEP, $info->presentation);
            @[$info->presentation, $info->horizontal] = $parts;
    ...
    public function set_ignoreempty($item, $ignoreempty = true) {
        $item->options = str_replace(INDIVIDUALFEEDBACK_FOURLEVELAPPROVAL_IGNOREEMPTY, '', $item->options);
        if ($ignoreempty) {
            $item->options .= INDIVIDUALFEEDBACK_FOURLEVELAPPROVAL_IGNOREEMPTY;
        }
    }

    public function ignoreempty($item) {
        if (strstr($item->options, INDIVIDUALFEEDBACK_FOURLEVELAPPROVAL_IGNOREEMPTY)) {
            return true;
        }
        return false;
    }
    ...
    public function set_negativeformulated($item, $negativeformulated = true) {
        $item->options = str_replace(INDIVIDUALFEEDBACK_FOURLEVELAPPROVAL_NEGATIVEFORMULATED, '', $item->options);
        if ($negativeformulated) {
            $item->options .= INDIVIDUALFEEDBACK_FOURLEVELAPPROVAL_NEGATIVEFORMULATED;
        }
    }

    public function negativeformulated($item) {
        if (strstr($item->options, INDIVIDUALFEEDBACK_FOURLEVELAPPROVAL_NEGATIVEFORMULATED)) {
            return true;
        }
        return false;
    }
    ...
    public function set_hidenoselect($item, $hidenoselect = true) {
        $item->options = str_replace(INDIVIDUALFEEDBACK_FOURLEVELAPPROVAL_HIDENOSELECT, '', $item->options);
        if ($hidenoselect) {
            $item->options .= INDIVIDUALFEEDBACK_FOURLEVELAPPROVAL_HIDENOSELECT;
        }
    }

    public function hidenoselect($item) {
        if (strstr($item->options, INDIVIDUALFEEDBACK_FOURLEVELAPPROVAL_HIDENOSELECT)) {
            return true;
        }
        return false;
    }
