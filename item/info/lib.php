...
class individualfeedback_item_info extends individualfeedback_item_base {
    ...
    public function build_editform($item, $individualfeedback, $cm) {
        ...
        if ($individualfeedback->anonymous == INDIVIDUALFEEDBACK_ANONYMOUS_NO || $item->presentation == self::MODE_RESPONSETIME) {
        ...
        //build the form
        $this->item_form = new individualfeedback_info_form('edit_item.php',
                                                  array('item'=>$item,
                                                  'common'=>$commonparams,
     ...
     * @return string
     */
    protected function get_current_value($item, $individualfeedback, $courseid) {
        global $DB;
        switch ($item->presentation) {
            case self::MODE_RESPONSETIME:
                if ($individualfeedback->anonymous != INDIVIDUALFEEDBACK_ANONYMOUS_YES) {
    ...
    public function complete_form_element($item, $form) {
        if ($form->get_mode() == mod_individualfeedback_complete_form::MODE_VIEW_RESPONSE) {
    ...
    public function get_data_for_external($item) {
        global $DB;
        $individualfeedback = $DB->get_record('individualfeedback', array('id' => $item->individualfeedback), '*', MUST_EXIST);
        // Return the default value (course name, category name or timestamp).
        return $this->get_current_value($item, $individualfeedback, $individualfeedback->course);
    }
