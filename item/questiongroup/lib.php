...
class individualfeedback_item_questiongroup extends individualfeedback_item_base {
    ...
    public function build_editform($item, $individualfeedback, $cm) {
        ...
        $lastposition = $DB->count_records('individualfeedback_item', ['individualfeedback' => $individualfeedback->id]);

        // Get all items to identify positions inside this group
        $allitems = $DB->get_records('individualfeedback_item', ['individualfeedback' => $individualfeedback->id], 'position');

        // Find the questiongroupend for this group
        $groupendposition = null;
        if ($item->id) {
            $groupend = $DB->get_record(
                'individualfeedback_item',
                ['individualfeedback' => $individualfeedback->id, 'typ' => 'questiongroupend', 'dependitem' => $item->id]
            );
            if ($groupend) {
    ...
    public function save_item() {
        ...
        $oldposition = null;
        if ($item->id) {
            $olditem = $DB->get_record('individualfeedback_item', ['id' => $item->id]);
            if ($olditem) {
        ...
        $params = ['individualfeedback' => $item->individualfeedback];
        ...
        $newitem = $DB->get_record('individualfeedback_item', ['id' => $item->id]);

        // SFSUBM-20 - only create a end group if there isn't one yet.
        $params = ['individualfeedback' => $newitem->individualfeedback, 'typ' => 'questiongroupend',
    ...
    private function move_entire_group($groupitem, $oldposition, $newposition) {
        ...
        $groupend = $DB->get_record(
            'individualfeedback_item',
            ['individualfeedback' => $groupitem->individualfeedback, 'typ' => 'questiongroupend', 'dependitem' => $groupitem->id]
        );
        ...
        $allitems = $DB->get_records(
            'individualfeedback_item',
            ['individualfeedback' => $groupitem->individualfeedback],
            'position'
        );
        ...
        foreach ($newpositions as $itemid => $pos) {
            $DB->set_field('individualfeedback_item', 'position', 10000 + $pos, ['id' => $itemid]);
        }

        // Then set the final positions
        foreach ($newpositions as $itemid => $pos) {
            $DB->set_field('individualfeedback_item', 'position', $pos, ['id' => $itemid]);
        }
    }
    ...
    public function print_analysed($item, $itemnr = '', $groupid = false, $courseid = false) {
        echo html_writer::start_tag('div', ['class' => 'questiongroup_analysed', 'id' => 'questiongroup_' . $item->id]);
        echo html_writer::tag('span', $item->name, ['class' => 'h3']);
    }
    ...
    public function print_overview_groups($item, $itemnr = '', $groupid = false, $courseid = false) {
        ...
        if (!$questions = $this->get_question_in_group($item)) {
            ...
            if (!$canprint) {
                ...
                echo "</table>";
                $graphdata = [];
                $graphdata['series_labels1'] = [$totalaverage];
                $graphdata['series_labels2'] = [$totalselfaverage];
                $graphdata['series1'] = [$totalaverage];
                $graphdata['series2'] = [$totalselfaverage];
    ...
    public function print_comparison_groups($item, $allindividualfeedbacks, $itemnr = '', $groupid = false, $courseid = false) {
        ...
        if (!$questions = $this->get_question_in_group($item)) {
            ...
            foreach ($allindividualfeedbacks as $oneindividualfeedback) {
                $alldata[$oneindividualfeedback->id] = [];
    ...
    public function excelprint_detail_groups(
        &$worksheet,
        $row_offset,
        $xls_formats,
        $item,
    ...
    public function excelprint_overview_groups(
        &$worksheet,
        $row_offset,
        $xls_formats,
        $item,
    ...
    public function excelprint_comparison_groups(
        &$worksheet,
        $row_offset,
        $xls_formats,
        $item,
    ...
    public function edit_actions($item, $individualfeedback, $cm) {
        ...
        $actions['delete'] = new action_menu_link_secondary(
            new moodle_url('/mod/individualfeedback/edit.php', ['id' => $cm->id, 'deleteitem' => $item->id, 'sesskey' => sesskey()]),
            new pix_icon('t/delete', $strdelete, 'moodle', ['class' => 'iconsmall', 'title' => '']),
            $strdelete,
            ['class' => 'editing_delete questiongroup', 'data-action' => 'delete']
        );
    ...
    public function get_question_in_group($item) {
        ...
        [$where, $qparams] = $DB->get_in_or_equal($qtypes, SQL_PARAMS_NAMED);
        ...
        $params = ['individualfeedback' => $item->individualfeedback, 'startposition' => $item->position, 'itemid' => $item->id];
...
class individualfeedback_item_questiongroupend extends individualfeedback_item_base {
     ...
     */
    public function complete_form_element($item, $form) {
        // Put end-of-group text in the label so it appears in "Move this question" (e.g. "After End of question group").
        $endlabel = html_writer::tag(
            'span',
            get_string('end_of_questiongroup', 'individualfeedback'),
            ['class' => 'individualfeedback_questiongroupend', 'id' => 'individualfeedback_item_' . $item->id]
        );
        $form->add_form_element(
            $item,
            ['static',
